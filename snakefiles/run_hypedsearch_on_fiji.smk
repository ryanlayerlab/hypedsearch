...
# Add singularity image to config
config["singularity_image"] = "hypedsearch_latest.sif"
...
rule assign_confidence_via_singularity:
    input:
        lambda wildcards: [
    ...
    output:
        "{out_dir}/assign-confidence.target.txt"
    params:
        comet_outputs = lambda wc: [
            f"/results/{sample}.comet.target.txt"
            for sample in config.samples
        ]
    shell:
        "singularity exec "
        ...
        "--overwrite T "
        "--list-of-files T "
        "{params.comet_outputs} "
        "&& rm /results/assign-confidence.params.txt " 
        "&& rm /results/assign-confidence.log.txt'" 

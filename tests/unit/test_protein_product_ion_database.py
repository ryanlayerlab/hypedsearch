...
# class Test_create_protein_table:
#     @staticmethod
#     def test_smoke():
#         db = ProteinProductIonDb(
#             db_path=MEMORY,
#             protein_obj=DbProtein,
#             product_ion_obj=DbProductIon,
#             protein_table_name="proteins",
#             product_ion_table_name="product_ions",
#             min_k=1,
#             max_k=2,
#             charges=[1],
#             ion_types=[IonTypes.B_ION_TYPE],
...
class Test_seq_to_ion:
    @staticmethod
    def test_smoke():
        seq = "ACD"
        protein_ids = [1, 2]
        ion = DbKmer.seq_to_ion(
            seq=seq,
            protein_ids=protein_ids,
        )
        assert ion.seq == seq
        assert ion.protein_ids == "1,2"
...
class Test_create_db:
    ...
    def test_smoke():
        db_proteins = [
            Peptide(seq="ACD", id=1, name="prot1"),
            Peptide(seq="PAC", id=2, name="prot2"),
            Peptide(seq="PAC", id=3, name="prot3"),
        ]
        uniq_kmer_to_protein_map = get_uniq_kmer_to_protein_map(
            proteins=db_proteins, min_k=3, max_k=3
        )
        ...
        kmers = db.get_product_ions()
        assert len(kmers) == 2
        assert DbKmer(seq="ACD", aa_mass=289.0732413, protein_ids="1") in kmers
        assert DbKmer(seq="PAC", aa_mass=271.0990622, protein_ids="2,3") in kmers

...
def create_sqllite_database(
    database_file_path,
    sqllite_database_path,
    max_peptide_length,
    digest_left,
    digest_right,
...
def relative_abundance_filtering(mass_to_charges, abundances, rel_abundance_cutoff):
    """
    ...
    mass_abundances = zip(mass_to_charges, abundances)
    filtered_mass_abundances = [x for x in mass_abundances if x[1] >= min_value]
    mass_to_charges = [float(x) for x, _ in filtered_mass_abundances]
    abundances = [float(x) for _, x in filtered_mass_abundances]

    return (mass_to_charges, abundances)


def select_highest_abundance_peaks(mass_to_charges, abundances, num_peaks):
    """
    Select the top N=num_peaks highest abundant mass-to-charge ratios
    """
    assert (
        isinstance(num_peaks, int) and num_peaks > 0
    ), f"The number of peaks to select should be an integer > 0. Received {num_peaks}"
    mass_abundances = zip(mass_to_charges, abundances)
    mass_abundances = sorted(mass_abundances, key=lambda x: x[1], reverse=True)[
        :num_peaks
    ]  # sort by abundance and ordered highest to lowest and subset to first num_peaks
    mass_abundances.sort(key=lambda x: x[0])  # sort by mass from lowest to highest
    mass_to_charges = [float(x) for x, _ in mass_abundances]
    abundances = [float(x) for _, x in mass_abundances]
    return (mass_to_charges, abundances)


def get_precursors(spectra_file_path, number_peaks, number_decimal_places):
    if not file_exists(spectra_file_path):
        print(
            "File {} not found. Please make sure that this file exists".format(
                spectra_file_path
            )
        )
        return
    ...
    for precursor_id, content in enumerate(filecontents):
        ...
        # ERIK: why is this 'if' statement here
        if not len(content["precursorList"]["precursor"]) or not len(
            content["precursorList"]["precursor"][0]["selectedIonList"]["selectedIon"]
        ):
            precursor_mass = max(
                masses
            )  # TODO: masses is used here but initialized below
            precursor_charge = 1
        else:
            precursor_mass = float(
                content["precursorList"]["precursor"][0]["selectedIonList"][
                    "selectedIon"
                ][0]["selected ion m/z"]
            )
            if (
                "peak intensity"
                in content["precursorList"]["precursor"][0]["selectedIonList"][
                    "selectedIon"
                ][0]
            ):
        ...
        rounded_precursor_mass = round(precursor_mass, number_decimal_places)
        ...
        if number_peaks > 0:
            adjusted_masses, adjusted_abundances = select_highest_abundance_peaks(
                masses, abundances, number_peaks
            )
        elif relative_abundance_filter > 0:  # TODO: this isn't set anywhere
            while relative_abundance_filter > 1:
                relative_abundance_filter /= 100
            adjusted_masses, adjusted_abundances = relative_abundance_filtering(
                masses, abundances, relative_abundance_filter
            )
        fragments = []
        for fragment_id, adjusted_mass in enumerate(adjusted_masses):
            ...
            fragment = Fragment(
                ...
                precursor_mass=rounded_precursor_mass,
                precursor_charge=precursor_charge,
                mz_value=rounded_mz_value,
                abundance=rounded_abundance,
            )
            fragments.append(fragment)
        precursor = Precursor(
            id=precursor_id,
            description=precursor_description,
            mass=rounded_precursor_mass,
            charge=precursor_charge,
            retention_time=retention_time,
            abundance=precursor_abundance,
            fragments=fragments,
        )
...
def get_all_precursors(spectra_file_paths, number_peaks, number_decimal_places):
    all_precursors = []
    for spectra_file_path in spectra_file_paths:
        precursors = get_precursors(
            spectra_file_path, number_peaks, number_decimal_places
        )
...
def get_output_row_from_aligned_peptide(spectrum_id, aligned_peptide):
    ...
    left_proteins, right_proteins = (
        aligned_peptide.left_proteins,
        aligned_peptide.right_proteins,
    )
    ...
    precursor_mass, precursor_charge = str(aligned_peptide.precursor_mass), str(
        aligned_peptide.precursor_charge
    )
    total_mass_error = str(aligned_peptide.total_mass_error)
    left_protein_string, right_protein_string = get_protein_strings(
        left_proteins, right_proteins
    )
...
def write_aligned_peptides_to_disk(
    aligned_peptides, output_folder_path, output_file_name
):
    ...
    with open(output_file_path, "w") as t:
        ...
        for aligned_peptide in aligned_peptides:
            output_row = get_output_row_from_aligned_peptide(
                spectrum_id, aligned_peptide
            )
            t.write(output_row)
            spectrum_id += 1
...
def get_experiment_parameters(args: dict):
    all_precursors = get_all_precursors(
        args["spectra_file_paths"], args["number_peaks"], args["number_decimal_places"]
    )
    sqllite_database = None
    if args["create_sqllite_database"]:
        sqllite_database = create_sqllite_database(
            args["database_file_path"],
            args["sqllite_database_path"],
            args["max_peptide_length"],
            args["digest_left"],
            args["digest_right"],
            args["number_decimal_places"],
        )
    else:
        sqllite_database = get_existing_sqllite_database(
            args["sqllite_database_path"], args["max_peptide_length"]
        )
...
def handle_results(args, aligned_peptides):
    ...
    write_aligned_peptides_to_disk(
        aligned_peptides, output_folder_path, output_file_name
    )
...
def run(args: dict):
    ...
    aligned_peptides = get_aligned_peptides(experiment_parameters)

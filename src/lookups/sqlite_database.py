...
class Sqllite_Database:
    ...
    def insert_proteins(self, proteins: List[Protein]):
        ...
        for protein_num, protein in enumerate(proteins):
            protein_data.append([protein_num, protein.desc, protein.seq])
        self.cursor.executemany("INSERT INTO proteins VALUES(?, ?, ?)", protein_data)
        self.connection.commit()
    ...
    def create_kmer_table_rows_for_protein(
        protein_id: int,
        protein: str,
        max_kmer_len: int,
        charges_to_consider: List[int] = [1, 2],
        amino_acid_mass_lookup: Dict[str, float] = AMINO_ACID_MASSES,
    ):
        """
        Takes the place of db_make_set_for_protein_digest.
        Previously there was digest_left & digest_right
        """
        data = []

...
class Config(dict):
    ...
    def _check_config(self):
        if not pathlib.Path(self["spectra_folder_path"]).is_dir():
            raise InvalidConfigFile(
                f'Spectra directory {self["spectra_folder_path"]} is not a vald directory'
            )
        if not pathlib.Path(self["database_file_path"]).is_file():
            raise InvalidConfigFile(
                f'Database file {self["database_file_path"]} is not a valid file'
            )
        if not pathlib.Path(self["output_folder_path"]).is_dir():
            raise InvalidConfigFile(
                f'Output directory {self["output_folder_path"]} is not a valid directory'
            )
        if self["number_peaks"] > 0 and self["relative_abundance"] > 0:
            warnings.warn(
                "The two peak filtering criteria are both set to non-zero values.\n"
    ...
    def __getitem__(self, key):
        ...
        if item is None:
            raise ConfigParamNotFound(
                f"Did not find parameter {key} in the config file"
            )
        return item
        return item
